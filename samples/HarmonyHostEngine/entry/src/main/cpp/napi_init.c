// HarmonyHostEngine C shim（引擎实验模板，由 samples/HarmonyHost 复制）：
// 与共享 C 节点 API 宿主的唯一差异是新增 initEngine 通道；passNodeContent/popPage 保留不动。
// 职责：
...
typedef int (*harmony_engine_init_t)(void* env, void* bridgeValue);
...
// ArkTS 引擎实验通道：把引擎桥对象交给 libapp.so（HarmonyEngineInit）。
// 桥对象上 C# 调 init(onEvent)/applyCommands(cmds)，引擎经注入的回调回流事件/量测。
static napi_value InitEngine(napi_env env, napi_callback_info info)
{
    ...
    if (argc < 1) {
        loge("initEngine", "missing engine bridge argument");
        return NULL;
    }

    harmony_engine_init_t engineInit = (harmony_engine_init_t)dlsym(g_app, "HarmonyEngineInit");
    if (!engineInit) {
        loge("dlsym HarmonyEngineInit", dlerror());
        return NULL;
    }

    int r = engineInit((void*)env, (void*)argv[0]);
    if (r == 0) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, LOG_TAG, "ArkTS engine bridge attached");
    } else {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, LOG_TAG, "HarmonyEngineInit failed: %{public}d", r);
    }
    return NULL;
}
...
static napi_value ModuleInit(napi_env env, napi_value exports)
{
    napi_property_descriptor desc[] = {
        ...
        {"initEngine", NULL, InitEngine, NULL, NULL, NULL, napi_default, NULL},
    };
