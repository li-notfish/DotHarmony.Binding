// <auto-generated>
// 由 apiGenerator.ts 生成（@ohos.* namespace 绑定路线）。
// 生成器输入：HarmonyOS SDK .d.ts → astParser → ComponentInfo → 本生成器
// </auto-generated>
#nullable enable
using System;
using System.Linq;
using System.Runtime.InteropServices;
using System.Text;
using System.Threading.Tasks;
using HarmonyOS.Bindings.Runtime;
using HarmonyOS.ArkUI;

namespace HarmonyOS.Bindings.Api.MultimodalInput;

/// <summary>
/// InputConsumer 绑定（@ohos.multimodalInput.inputConsumer）。
/// </summary>
public static unsafe partial class InputConsumer
{
    private const string ModuleName = "@ohos.multimodalInput.inputConsumer";

    private static NapiReference? _moduleRef;
    private static bool _loadAttempted;

    /// <summary>懒加载的 @ohos 模块对象（internal：同文件包装类的构造函数需要）</summary>
    internal static IntPtr Module
    {
        get
        {
            if (_moduleRef != null) return _moduleRef.Value;
            if (_loadAttempted)
                throw new InvalidOperationException($"{ModuleName} module failed to load (previous attempt)");
            _loadAttempted = true;

            var env = NapiEnv.Current;
            NativeNodeApi.napi_open_handle_scope(env, out var scope).ThrowIfFailed();
            try
            {
                foreach (var name in new[] { "=" + ModuleName, ModuleName })
                {
                    var utf8 = System.Text.Encoding.UTF8.GetBytes(name);
                    fixed (byte* p = utf8)
                    {
                        var status = NativeNodeApi.napi_load_module(env, p, out var module);
                        if (status == napi_status.napi_ok && module != IntPtr.Zero)
                        {
                            _moduleRef = new NapiReference(module);
                            break;
                        }
                        NativeNodeApi.napi_get_and_clear_last_exception(env, out _);
                    }
                }
            }
            finally
            {
                NativeNodeApi.napi_close_handle_scope(env, scope).ThrowIfFailed();
            }

            if (_moduleRef == null)
                throw new InvalidOperationException(
                    $"failed to load {ModuleName} via napi_load_module (tried with and without '=' prefix)");

            HiLog.Info("HarmonyHost", $"[multimodalInput.inputConsumer] module loaded via napi_load_module");
            return _moduleRef.Value;
        }
    }

    private static ReadOnlySpan<byte> _getAllSystemHotkeys => "getAllSystemHotkeys"u8;
    private static ReadOnlySpan<byte> _on => "on"u8;
    private static ReadOnlySpan<byte> _off => "off"u8;

    /// <summary>
    /// getAllSystemHotkeys
    /// </summary>
    public static Task<HotkeyOptions[]> GetAllSystemHotkeysAsync()
    {
        return NodeApi.CallMethodAsync(Module, _getAllSystemHotkeys, h => ValueConverter.ConvertArray(h, static e => new HotkeyOptions(e)));
    }

    /// <summary>
    /// on
    /// </summary>
    public static void On(string type, HotkeyOptions hotkeyOptions, IntPtr callback)
    {
        NodeApi.CallMethodVoid(Module, _on, type, NapiArg.Of(hotkeyOptions), callback);
    }

    /// <summary>
    /// off
    /// </summary>
    public static void Off(string type, HotkeyOptions hotkeyOptions, IntPtr callback)
    {
        NodeApi.CallMethodVoid(Module, _off, type, NapiArg.Of(hotkeyOptions), callback);
    }

    /// <summary>
    /// on
    /// </summary>
    public static void On(string type, KeyPressedConfig options, IntPtr callback)
    {
        NodeApi.CallMethodVoid(Module, _on, type, NapiArg.Of(options), callback);
    }

    /// <summary>
    /// off
    /// </summary>
    public static void Off(string type, IntPtr callback)
    {
        NodeApi.CallMethodVoid(Module, _off, type, callback);
    }

    private static readonly EventListenerRegistry _eventListeners = new();

    /// <summary>
    /// on(type, callback) 的类型化重载（回调经共享跳板进入 C#，任意参数类型自动转换）
    /// </summary>
    public static void On(string type, System.Action<HotkeyOptions> callback, HotkeyOptions hotkeyOptions)
    {
        _eventListeners.Add((type, callback),
            args => callback(new HotkeyOptions(args[0])),
            js => NodeApi.CallMethodVoid(Module, _on, type, js, NapiArg.Of(hotkeyOptions)));
    }

    /// <summary>
    /// off(type)：移除该事件类型的全部回调
    /// </summary>
    public static void Off(string type)
    {
        NodeApi.CallMethodVoid(Module, _off, type);
    }

    /// <summary>
    /// off(type, callback)：解除订阅（按 handler 匹配）
    /// </summary>
    public static void Off(string type, System.Action<HotkeyOptions> callback, HotkeyOptions hotkeyOptions)
    {
        _eventListeners.Remove((type, callback), js => NodeApi.CallMethodVoid(Module, _off, type, js, NapiArg.Of(hotkeyOptions)));
    }

    /// <summary>
    /// on(type, callback) 的类型化重载（回调经共享跳板进入 C#，任意参数类型自动转换）
    /// </summary>
    public static void On(string type, System.Action<global::HarmonyOS.Bindings.Api.MultimodalInput.KeyEventKeyEvent> callback, KeyPressedConfig options)
    {
        _eventListeners.Add((type, callback),
            args => callback(new global::HarmonyOS.Bindings.Api.MultimodalInput.KeyEventKeyEvent(args[0])),
            js => NodeApi.CallMethodVoid(Module, _on, type, js, NapiArg.Of(options)));
    }

    /// <summary>
    /// off(type, callback)：解除订阅（按 handler 匹配）
    /// </summary>
    public static void Off(string type, System.Action<global::HarmonyOS.Bindings.Api.MultimodalInput.KeyEventKeyEvent> callback)
    {
        _eventListeners.Remove((type, callback), js => NodeApi.CallMethodVoid(Module, _off, type, js));
    }

    /// <summary>
    /// 监听 hotkeyChange 事件（对应 on/off）
    /// </summary>
    public static event System.Action<HotkeyOptions> HotkeyChange
    {
        add
        {
            _eventListeners.Add(("hotkeyChange", value),
                args => value(new HotkeyOptions(args[0])),
                js => NodeApi.CallMethodVoid(Module, _on, "hotkeyChange", js));
        }
        remove
        {
            _eventListeners.Remove(("hotkeyChange", value), js => NodeApi.CallMethodVoid(Module, _off, "hotkeyChange", js));
        }
    }

    /// <summary>
    /// 监听 keyPressed 事件（对应 on/off）
    /// </summary>
    public static event System.Action<global::HarmonyOS.Bindings.Api.MultimodalInput.KeyEventKeyEvent> KeyPressed
    {
        add
        {
            _eventListeners.Add(("keyPressed", value),
                args => value(new global::HarmonyOS.Bindings.Api.MultimodalInput.KeyEventKeyEvent(args[0])),
                js => NodeApi.CallMethodVoid(Module, _on, "keyPressed", js));
        }
        remove
        {
            _eventListeners.Remove(("keyPressed", value), js => NodeApi.CallMethodVoid(Module, _off, "keyPressed", js));
        }
    }

}

/// <summary>
/// HotkeyOptions 实例包装（@ohos 命名空间内嵌套接口）。
/// 由 JsObject 持有 napi 强引用；Dispose 仅释放引用，JS 对象由 ArkTS GC 管理。
/// </summary>
public sealed partial class HotkeyOptions : JsObject
{
    public HotkeyOptions(IntPtr handle) : base(handle) { }
    private static ReadOnlySpan<byte> _preKeys => "preKeys"u8;
    private static ReadOnlySpan<byte> _finalKey => "finalKey"u8;
    private static ReadOnlySpan<byte> _isRepeat => "isRepeat"u8;
    /// <summary>
    /// preKeys
    /// </summary>
    public double[] PreKeys => ValueConverter.ConvertArray(GetPropertyRaw(_preKeys), static e => ValueConverter.Convert<double>(e));

    /// <summary>
    /// finalKey
    /// </summary>
    public double FinalKey => NativeValue.ToDouble(GetPropertyRaw(_finalKey));

    /// <summary>
    /// isRepeat
    /// </summary>
    public bool? IsRepeat => (bool?)NativeValue.ToBool(GetPropertyRaw(_isRepeat));

}

/// <summary>
/// KeyPressedConfig（@ohos 命名空间内嵌套纯数据接口，入参对象）。
/// </summary>
public sealed record KeyPressedConfig(
    double Key,
    double Action,
    bool IsRepeat
) : INapiRecord
{
    private static ReadOnlySpan<byte> _keyName => "key"u8;
    private static ReadOnlySpan<byte> _actionName => "action"u8;
    private static ReadOnlySpan<byte> _isRepeatName => "isRepeat"u8;
    void INapiRecord.WriteTo(NativeNodeApi.napi_env env, NativeNodeApi.napi_value obj)
    {
        var _keyV = NativeValue.From(Key);
        if (_keyV != IntPtr.Zero)
            NativeNodeApi.napi_set_named_property(env, obj, _keyName, _keyV);
        var _actionV = NativeValue.From(Action);
        if (_actionV != IntPtr.Zero)
            NativeNodeApi.napi_set_named_property(env, obj, _actionName, _actionV);
        var _isRepeatV = NativeValue.From(IsRepeat);
        if (_isRepeatV != IntPtr.Zero)
            NativeNodeApi.napi_set_named_property(env, obj, _isRepeatName, _isRepeatV);
    }
}
