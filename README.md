...
samples/dotnet/EssentialsApp/ M2.4 Essentials 验证（DeviceInfo/DeviceDisplay/AppInfo + 剪贴板授权回环）
...
- M2 全部完成（含 Essentials 平台实现首批：DeviceInfo/DeviceDisplay/AppInfo/Clipboard）：~~TSFN 异步层~~、~~codeGenerator 修复~~、~~@ohos.* 全量生成（438 模块/375 转正）~~、~~Promise→Task/AsyncCallback/.NET 事件/ArrayBuffer/Map~~、~~端到端模拟器验证~~、~~零分配调用路径~~、~~2.4 Essentials 首批（含剪贴板 user_grant 授权闭环，2026-09-13）~~
