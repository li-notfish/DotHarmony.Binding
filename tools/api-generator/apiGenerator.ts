...
/** 模块路径 → 子命名空间/子目录段（除末段外全部中间段，PascalCase）：bluetooth.a2dp → "Bluetooth"；
 *  multimedia.camera.picker → "Multimedia.Camera"；顶层模块（deviceInfo）→ ""（留在 Api 根）。
 *  产物文件与命名空间按此分组：Api/<sub 段路径>/X.cs、HarmonyOS.Bindings.Api[.Sub…] */
export function moduleSubNamespace(local: string): string {
...
export class ApiGenerator {
    ...
    /**
     * @ohos.deviceInfo.d.ts → { module: "@ohos.deviceInfo", className: "DeviceInfo", local: "deviceInfo", subNamespace: "" }
     * @ohos.bluetooth.a2dp.d.ts → { module: "@ohos.bluetooth.a2dp", className: "A2dp", local: "bluetooth.a2dp", subNamespace: "Bluetooth" }
     */
    static dtsToModuleInfo(dtsFileName: string): { module: string; className: string; local: string; subNamespace: string } {
        ...
        return { module: stem, className, local, subNamespace: moduleSubNamespace(local) };
    }
    ...
    private isKnownTypeIdent(id: string): boolean {
        return ApiGenerator.IDENT_ALLOWLIST.has(id)
            ...
            // 跨模块 FQN 现按源模块子命名空间分组（Api.Bluetooth.A2dp 形态）——按末段后缀匹配，
            // 误匹配只是把已知跨模块类型当作已知（不降级），方向安全
            || [...this.crossModuleWrappers].some(fqn => fqn === `global::HarmonyOS.Bindings.Api.${id}`
                || fqn.endsWith(`.${id}`))
    ...
    private emitHeader(lines: string[], moduleInfo: { module: string; className: string; local: string; subNamespace: string }, permissions: string[], needsTask: boolean, needsLinq: boolean): void {
        ...
        void needsLinq;
        lines.push('');
        // 命名空间按模块路径中间段分组：bluetooth.a2dp → HarmonyOS.Bindings.Api.Bluetooth
        lines.push(`namespace HarmonyOS.Bindings.Api${moduleInfo.subNamespace ? '.' + moduleInfo.subNamespace : ''};`);
        lines.push('');
