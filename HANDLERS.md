...
单测：`tests/dotnet/HarmonyGestureTests`（xunit，`dotnet test` 跑）——反射桥全链路、Swipe 方向映射、Grid 对齐偏移纯逻辑、Essentials 密度/方向映射、Preferences 编解码、Battery 枚举映射，共 62 用例（含漏斗纪律源码扫描闸）。
